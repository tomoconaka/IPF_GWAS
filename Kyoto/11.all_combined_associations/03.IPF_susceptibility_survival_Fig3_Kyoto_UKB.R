...
kyoto <- fread("/mnt/gpfs1/project/Interstitial_Pneumonia/ANALYSIS/tnakanishi/scratch/IPF.susceptibility.assoc.tsv")
...
kyoto <- kyoto %>% mutate(Ncases = 106, Ncontrols = 2731)
...
ALL <- ALL %>% mutate(variable = case_when(variable %in% c("PRS", "prs") ~ "1SD increase in PRS",
...
ALL %>% mutate(name = fct_relevel(variable, "Rare telomere variants carrier", "1SD increase in PRS", "1SD decrease in age/sex adjusted LTL",
  ...
  xlab("") + ylab("Odds Ratio (95% Confidence Interval)") + geom_text(aes(label=round(OR,1), y=OR, col=cohort, hjust = 0.5, vjust = 1.5), size=5) +
...
####################SURVIVAL
kyoto <- fread("/mnt/gpfs1/project/Interstitial_Pneumonia/ANALYSIS/tnakanishi/scratch/IPF.survival.assoc.tsv")
ukb <- fread("/mnt/gpfs1/project/Interstitial_Pneumonia/ANALYSIS/tnakanishi/UKB_scratch/IPF_survival.tsv")

kyoto <- kyoto %>% mutate(coef = case_when(variable == "age.x" ~ 10*coef,
                                           variable == "tel_sd" ~ -1*coef,
                                           TRUE ~ coef),
...
kyoto <- kyoto %>% mutate(HR = exp(coef),
  ...
  select(variable, HR, LL, UL, p, Beta, se)

kyoto <- kyoto %>% mutate(cohort = "Kyoto-ILD")

ukb <- ukb %>% mutate(Beta = case_when(variable == "AGE" ~ 10*coef,
                                       variable == "TS_sd" ~ -1*coef,
                                       TRUE ~ coef),
...
ALL <- ALL %>% mutate(variable = case_when(variable %in% c("PRS", "prs") ~ "1SD increase in PRS",
                                           variable %in% c("telomere_gene", "tel_gene") ~ "Rare telomere variants carrier",
                                           variable %in% c("tel_sd", "TS_sd")~ "1SD decrease in age/sex adjusted LTL",
                                           variable %in% c("age.x", "AGE") ~ "10 year increase in age at diagnosis",
...
library(forcats)
ALL %>% mutate(name = fct_relevel(variable, "Rare telomere variants carrier", "1SD increase in PRS", "1SD decrease in age/sex adjusted LTL",
                                  "10 year increase in age at diagnosis", "Male", "Ever smoker")) %>%
  ggplot(aes(x=cohort, y=HR, ymin=LL, ymax=UL, color=cohort)) +
  geom_pointrange(aes(col=cohort), lwd=0.8) + geom_hline(aes(fill=cohort), yintercept =1, linetype=2) +
  xlab("") + ylab("Hazard Ratio (95% Confidence Interval)") + geom_text(aes(label=round(HR,1), y=HR, col=cohort, hjust = 0.5, vjust = 1.5), size=5) +
...
ALL <- ALL %>% select(variable, HR, LL, UL, p, cohort, hetero_p)
